...
RUNS = 3
MAX_GPUS = 3
BYTE_COUNT = 1153241457 * 1712826111
...
data = read_my_file("../tacc_scripts/small.out")
data = data.split("~~~")
...
plt.yticks(np.arange(0, 32001, 2000))
...
# title
plt.title("Throughput—Small")

plt.tight_layout()
plt.savefig("../figures/throughput-small.pdf")
if SHOW_FIGURES:
...
# title
plt.title("Speedup—Small")

plt.tight_layout()
if SHOW_FIGURES:
    plt.show(block=BLOCK_ON_SHOW)
plt.savefig("../figures/speedup-small.pdf")
